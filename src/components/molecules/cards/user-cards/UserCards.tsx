...
function UserCards(props: CardProps) {
  ...
  const registerUser = async () => {
    ...
    try {
      const response = await axios.patch(`http://localhost:5000/users/patch/${props.title}/${userr.username}`,register,{
        headers: {
    ...
    <div className=" w-72 rounded overflow-hidden shadow-xl bg-white">
      ...
      <div className="px-6 py-4">
        ...
        <div className='flex justify-between'>
          ...
          <Button variant="outlined" size="small" onClick={registerUser}>
            Register
          </Button>
        </div>
      </div>
